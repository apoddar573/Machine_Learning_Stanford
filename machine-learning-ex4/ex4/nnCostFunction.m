...
for i = 1:m
	y_i = zeros(size(z3, 1), 1);
	y_i(y(i)) = 1;
	y_i = -y_i .* log(z3(:, i)) - (1-y_i).*log(1-z3(:, i));
	J = J + sum(y_i);
end
J = J/m;
n1 = size(Theta1, 2);
n2 = size(Theta2, 2);
reg_term = sum( sum( Theta1(:,2:n1) .^ 2) ) + sum(sum(Theta2(:, 2:n2) .^ 2) );
...
for i = 1:m
	...
	y_i = zeros(length(a_3), 1);
	y_i(y(i)) = 1;
	d_3 = a_3 - y_i;
	d_2 = Theta2'*d_3;
	d_2 = d_2(2:length(z_2)+1) .* sigmoidGradient(z_2);
